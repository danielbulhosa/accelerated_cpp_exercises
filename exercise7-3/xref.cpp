...
map<string, vector<int> > xref(istream& input, string_parser& find_words, const bool& unique_counts){
    string line;
    int line_number = 0;
    ...
    while(getline(input, line)){
        ...
        for(const string& word: words){
            // If we're only counting unique lines and we just saw the same word in the current line.
            if(unique_counts &&
               !ret[word].empty() && // Needed to add this check to avoid indexing empty list in next condition.
               line_number == ret[word][ret[word].size() - 1]){
                continue;
            }
            else{
